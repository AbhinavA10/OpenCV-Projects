...
def processImage (image): #image is the frame sent by the video capture
   ...
   #3. Use the Canny Edge Detection algorithim to detect edges. 
   ...
   # Define thresholds for how strong the edge must be.
    ...
    #returning 'edges' at this point would return the image result of cv2.Canny that show edges       
    #return edges
    
    #4. Next create a masked edges image using cv2.fillPoly()
    ...
    #returning 'masked_edges' at this point would return the image showing edges in the trapezoid area.
    #return masked_edges

    #5. Use the Hough Line Transform to detect a line from the above, possibly circular, edges
    ...
    #6. Iterate over "lines" and draw lines on a blank image
    for line in lines:
...
out = cv2.VideoWriter(FILE_OUTPUT,-1, 20.0, (int(width),int(height)))
